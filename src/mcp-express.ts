...
// ── SSE Transport for Claude Remote Connectors ──────────────────────────────
...
app.listen(PORT, () => {
    console.log(`Claude Remote Connector (SSE) running on port ${PORT}`);
    console.log(`Connect via Claude UI at: http://localhost:${PORT}/mcp/sse`);
});
